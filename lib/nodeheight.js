        ...
        visitNode : function(path){
            ...
            types.eachField(path.node,function(name,value){
                if(value && value.type ){
                    ...
                    path.node.descendants.push({hash : value.hash, node : value});
                    value.parent = path.node;
                }
                else if (_.isArray(value)){
                    value.map(function(eachValue){
                        fieldHeights.push(eachValue.treeHeight);
                        path.node.descendants = path.node.descendants.concat(eachValue.descendants);
                        path.node.descendants.push({hash : eachValue.hash, node : eachValue});
                        eachValue.parent = path.node;
                    });
                }
            });
